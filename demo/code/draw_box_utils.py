...
def draw_box(image, boxes, classes, scores, category_index, thresh=0.45, line_thickness=2):
    ...
    for box, color in box_to_color_map.items():
        ...
        (left, right, top, bottom) = (xmin * 1, xmax * 1,
                                      ymin * 1, ymax * 1)
        pos.append(((left+right)/2,(top+bottom)/2)) # pos keeps the (x,y) cooridinate of the center
        draw.line([(left, top), (left, bottom), (right, bottom),
                   (right, top), (left, top)], width=line_thickness, fill=color)
        draw_text(draw, box_to_display_str_map, box, left, right, top, bottom, color)
    with open('../result/result.txt','w+') as f:
        for i, j in zip(pos,tag):
            s = str(i[0]) + " " + str(i[1]) + " "
            f.write(s)
            f.write(str(j))
            f.write('\n')
